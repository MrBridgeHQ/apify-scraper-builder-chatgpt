...
This file assumes you've read the main `SKILL.md`. It expands Path C specifics. The deeper doctrine layer (anti-bot, tool ladder, selectors, error handling, fixtures) is treated as a separate concern and cross-referenced - do not duplicate.
...
The full tool-ladder reasoning (cost-per-result, IP economics, escalation triggers) belongs to your scraping doctrine layer. Consult it before picking when uncertain. **Default to Cheerio and escalate on observed failure** - using Playwright for static HTML burns 10× compute for zero benefit.
...
Memory tiers and cost (per your Apify-patterns reference layer):
...
Full editor catalog (`stringList`, `requestListSources`, `proxy`, `select`, etc.) belongs to your Apify-patterns reference layer (input_schema.json).
...
Full schema validation doctrine (including JSON-Schema constraints, error messages, transformations) belongs to your selector-strategy reference layer (Schema validation).
...
For multi-event scrapers (e.g. category-page scan + product-detail-fetch), declare each event separately and charge per logical operation. Pricing-strategy doctrine belongs to your monetization reference layer - **do not guess prices**.
...
For **Playwright** templates (`ts-crawlee-playwright-chrome`), swap `FROM apify/actor-node:20` → `FROM apify/actor-node-playwright-chrome:20`. For **Camoufox**, use `apify/actor-node-playwright-firefox:20` then add `RUN npm install camoufox-js`. Full base-image catalog belongs to your Apify-patterns reference layer (Build images and dependencies).
...
The graceful-exit pattern (push error to dataset → SUCCEEDED) is non-negotiable. Full doctrine belongs to your error-handling reference layer.
...
Full selector strategy + schema validation patterns belong to your selector-strategy reference layer.
...
Full PPE doctrine belongs to your monetization and error-handling reference layers.

## Anti-bot escalation inside Path C

Cross-ref: your anti-bot strategy reference layer has the full 5-rung escalation ladder.
...
| Camoufox detected at scale | Managed API (Firecrawl / ZenRows / ScrapFly) - see your managed-APIs reference layer |
...
For local development: **never hit the target site from the local machine.** Tests use captured fixtures.
...
Fixtures live in `tests/fixtures/`, scrubbed of PII. Capture them once via the Apify Console (run the scraper, save the HTML response) and commit to git. Full pattern + fixture-scrubbing belongs to your testing-fixtures reference layer.
...
- **Tool ladder reasoning (Cheerio vs Playwright vs Camoufox at deeper level)** → your tool-ladder reference layer
- **Anti-bot escalation tactics, vendor signatures** → your anti-bot strategy reference layer
- **Diagnostic (Wappalyzer-first, hidden APIs)** → your diagnostic reference layer
- **PPE doctrine** → your monetization reference layer
- **README / Store description / pricing copy** → your Apify Actor content reference layer
- **Managed APIs (Firecrawl/ZenRows/ScrapFly) when Crawlee economics break** → your managed-APIs reference layer
