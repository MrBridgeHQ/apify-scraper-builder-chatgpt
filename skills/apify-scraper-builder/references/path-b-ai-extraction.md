...
- ❌ The site has an internal JSON API - use it.

The first move on **any** target is still the diagnostic - internal API first, schema.org markup second, selectors third, LLM fourth.
...
| Multi-step extraction with chained calls | Custom orchestration | Beyond this skill's scope - see your LLM API reference |
...
Reduces system-prompt cost to ~10% of normal after the first call. See your LLM API reference for full caching details.
...
Full discussion + decision criteria belong to your Apify monetization reference layer (LLM-cost handling).
...
- **LLM API patterns (prompt caching, model choice, batch, citations)** → your LLM API reference layer
- **LLM cost-handling strategies (1A/1B/2/3) full doctrine** → your Apify monetization reference layer (LLM-cost handling)
- **Schema.org / `[itemprop]` selectors (the cheap escape from AI extraction)** → your selector-strategy reference layer
